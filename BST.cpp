...
/*void BST::InsertH(int key)
{
	//newnode = make_shared<BSTNode>(key);
	shared_ptr<BSTNode> newNode = make_shared<BSTNode>(key);
	//return insert(root_, newNode);//shared_ptr<BSTNode> node);

}

void BST::insert(shared_ptr<BSTNode> node, shared_ptr<BSTNode> newNode)
{
    if (node == nullptr)
    {
    	node = newNode; 
...
int BSTNode::Height(shared_ptr<BSTNode> t)
{
	if (t)
		return t->height_;
	else
...
int BSTNode::max(int leftH, int rightH)
{
